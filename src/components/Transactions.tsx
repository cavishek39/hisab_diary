    ...
    <div className="space-y-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="opacity-40 text-sm font-medium uppercase tracking-widest">History of your financial activities</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          <div className="relative flex-1 md:w-64">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-3 bg-text-main/5 border border-card-border rounded-2xl focus:border-brand-emerald outline-none font-bold"
            />
          </div>
          <div className="flex bg-text-main/5 border border-card-border p-1 rounded-2xl">
            {['All', 'Expense', 'Income'].map((t) => (
              <button
                ...
                className={cn(
                  ...
                  filterType === t ? "bg-text-main text-bg-main shadow-lg" : "opacity-40 hover:opacity-100"
                )}
              >
                {t}
              </button>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-card-border">
              {filteredTransactions.map(t => {
                ...
                return (
                  <tr key={t.id} className="group hover:bg-text-main/[0.01] transition-colors">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        <div className={cn(
                          "w-10 h-10 rounded-2xl flex items-center justify-center font-bold text-xs",
                          t.type === 'Income' ? "bg-brand-emerald/10 text-brand-emerald" : "bg-text-main/5 opacity-40"
                        )}>
                    ...
                    <td className="px-8 py-6">
                      <span className="px-3 py-1 bg-text-main/5 rounded-full text-[10px] font-black uppercase tracking-widest opacity-40 border border-card-border">
                        {t.category}
                      </span>
                    </td>
                    ...
                    <td className="px-8 py-6 text-right">
                      <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button 
                          ...
                          className="p-3 hover:bg-black/5 dark:hover:bg-white/10 rounded-xl opacity-40 hover:opacity-100 transition-all active:scale-95"
